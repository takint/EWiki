...
{
    public class RepositoryBase<TEntity> : IRepository<TEntity> where TEntity : class, IObjectState
    {
        ...
        {
            entity.ObjState = ObjectState.Added; ;
        ...
        {
            entity.ObjState = ObjectState.Modified;
            _dbSet.Attach(entity);
            _context.SyncObjectState(entity);
        }
        ...
        {
            entity.ObjState = ObjectState.Deleted;
            _dbSet.Attach(entity);
            _context.SyncObjectState(entity);
        }
        ...
        {
            return _unitOfWork.RepositoryBase<T>();
        }
        ...
        private void SyncObjectGraph(object entity) // scan object graph for all 
        {
            ...
            if (objectState != null && objectState.ObjState == ObjectState.Added)
            ...
            foreach (var prop in entity.GetType().GetProperties())
            {
                ...
                {
                    // discovered entity with ObjectState.Added, sync this with provider e.g. EF
                    if (trackableRef.ObjState == ObjectState.Added)
                        _context.SyncObjectState((IObjectState)entity);
